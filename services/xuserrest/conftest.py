...
#     def _create_user(role_list=None, session=None):
...
#         username = f"pytest_fixture_{uuid.uuid4().hex[:8]}"
...
#     # cleanup (can stay with requests or convert to session if needed)
#     for user_id in created_user_ids:
#         requests.delete(
#             f"{ranger_config['base_url']}/xusers/users/{user_id}",
#             params={"forceDelete": "true"},
...
#     def _create_keyadmin(session=None):
...
#         # promote to keyadmin (can stay requests)
#         requests.put(
#             f"{ranger_config['base_url']}/xusers/secure/users/roles/{user_id}",
#             json={"vXStrings": [{"value": "ROLE_KEY_ADMIN"}]},
#             auth=("keyadmin", "rangerR0cks!"),
