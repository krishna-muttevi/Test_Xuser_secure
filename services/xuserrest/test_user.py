...
class TestUsers:
    ...
    def test_get_users(self, role, request):
        if role == "keyadmin":
            ...
            auth_user, auth_id = request.getfixturevalue("temp_secure_user")([role])
            auth = (auth_user["name"], "Test@123")
    ...
    def test_get_user_by_id(self, auth_role, user_role, request):
        ...
        if user_role == "same_user":
            ...
            user, user_id = request.getfixturevalue("temp_keyadmin_user")()
        else:
            user, user_id = request.getfixturevalue("temp_secure_user")([user_role])
